...
# suffix=paste('_10feats_importance', args[1], sep='')
suffix=paste('_tiny_test', args[1], sep='')
...
sub.df.pure = sample_n(df.pure, 10)
# sub.df.pure = df.pure[1:10,]

df.weak = colorify.full(df_weak)
df.weak$type = df_weak$type
# sub.df.weak = df.weak[1:10,]
sub.df.weak = sample_n(df.weak[df.weak$type=='weak',], 10)
sub.df.strong = sample_n(df.weak[df.weak$type=='strong',], 10)

df.master = rbind(sub.df.pure, sub.df.weak, sub.df.strong)

# df.master = rbind(df.pure, df.weak)
phot.master = df.master[,1:10]
# phot.master = phot.master[,!names(phot.master) %in% remove.cols]

# df.master = read.csv(paste(indir,"/iden_err_rf.csv", sep=""))
# phot.master = df.master[,1:19]

print(str(phot.master))

# weak.master = df.master$blend
# strong.master = df.master$blend
# strong.master[df.master$type == 'weak'] = 0

weak.master = df.master$type
# weak.master[weak.master == 'weak'] = 'blend'
# weak.master[weak.master == 'strong'] = 'blend'
weak.master = as.factor(weak.master)

strong.master = df.master$type
strong.master[strong.master == 'weak'] = 'pure'
# strong.master[strong.master == 'strong'] = 'blend'
...
###############################################
ntree = 500
out.rf = randomForest(x=phot.master,y=weak.master,ntree=ntree, norm.votes=FALSE, do.trace=TRUE, importance=T)
rcont = list("method"=out.rf)
save(rcont, file=paste(outdir,"/rfobj", suffix, '_w.Rda', sep=''))

out.rf = randomForest(x=phot.master,y=strong.master,ntree=ntree, norm.votes=FALSE, do.trace=TRUE, importance=T)
rcont = list("method"=out.rf)
