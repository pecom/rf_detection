...
phot.master = select(df.master, -H, -J, -Y)
...
vali.phot = select(weak.vali, -H, -J, -Y)
...
for (run.num in 1:4){
	...
	for (b in bands){
		...
		band.run.df = select(phot.run, -FLUX_RADIUS)

		col.seq = og.bands[-match(remove.cols, og.bands)]
		band.master = colorify.funky(band.run.df, col.seq)

		# band.master[,b] = phot.run[,b]
		if (b!='FLUX_RADIUS'){
			band.master$FLUX_RADIUS = phot.run$FLUX_RADIUS
		}
		band.master$u = phot.master$u
