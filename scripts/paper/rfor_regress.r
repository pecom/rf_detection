...
suffix=paste('_10feats_regress', args[1], sep='')
...
###############################################
ntree = 500
out.rf = randomForest(x=phot.master,y=weak.master,ntree=ntree,do.trace=TRUE)
rcont = list("method"=out.rf)
save(rcont, file=paste(outdir,"/rfobj", suffix, '_w.Rda', sep=''))

out.rf = randomForest(x=phot.master,y=strong.master,ntree=ntree,do.trace=TRUE)
rcont = list("method"=out.rf)
