...
def process_rf_vali(fname, bflag, star_cut=.4, rf_bands=None, data_dir='$PSCRATCH/data/run1'):
    ...
    with fits.open(f'{data_dir}/{fname}.fits') as hdul:
        ...
        phot_err = np.zeros_like(phot_data)
        for i,rb in enumerate(rf_bands):
            (blend_vali[star_vali < 1] > 0).astype(int)
            if rb == bflag:
...
if __name__ == "__main__":
    ...
    np.savetxt(f'{out_dir}/training_pure_messy.csv', training_pure, delimiter=',', header=csv_headsr, comments='')
    np.savetxt(f'{out_dir}/training_blend_messy.csv', training_blend, delimiter=',', header=csv_headsr, comments='')
    np.savetxt(f'{out_dir}/vali_weak_messy.csv', vali_weak, delimiter=',', header=csv_headsr, comments='')
    np.savetxt(f'{out_dir}/vali_strong_messy.csv', vali_strong, delimiter=',', header=csv_headsr, comments='')
